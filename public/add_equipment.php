...
if (isset($_GET['project_id'])) {
    ...
    $stmt = $pdo->prepare("
        SELECT form_no, quantity, assigned_to, COUNT(*) as saved_count
        FROM project_forms
        WHERE project_id = ?
        GROUP BY form_no, quantity, assigned_to
    ");
    $stmt->execute([$project_id]);
        ...
        fetch('add_equipment.php?project_id=' + encodeURIComponent(projectId))
            ...
            .then(data => {
                if (!data.success) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="7" class="border border-gray-300 px-4 py-8 text-center text-red-500">
                                ${data.message}
                            </td>
                        </tr>`;
                    return;
